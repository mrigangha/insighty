...
class VisitorSession(Base):
    ...
    # ✅ backref only (NO FK column here)
    lead = relationship(
        "Lead",
...
class VisitorEvent(Base):
    ...
    visitor_session_id = Column(
        Integer, ForeignKey("visitor_sessions.id"), index=True, nullable=False
    )
    ...
    )  # page_view, click, time_spent
    path = Column(String(500), nullable=True)  # URLs can be long
...
class Domain(Base):
    ...
    domain_name = Column(String, primary_key=True)  # e.g., "https://clientsite.com"
